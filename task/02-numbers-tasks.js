...
function isPrime(n) {
    let i = 2;
    let PrimeNumbers = [];
    while (i <= n) { //Разложение системы счисления на простые множители 
        if (n%i == 0) {
          PrimeNumbers.push(i);
          n /=i; 
        } else {i++};
    };
    if (PrimeNumbers.length == 1) {return true} else {return false};
}
...
function toNumber(value, def) {
    value = +value;
    if (isNaN(value) === true) {return def} else {return value};
}
